...
\centering
  ...
  \end{center}
  \caption{Solución considerada 3}\label{Fig:HW} 
\end{figure}

\item[4 -] SBC + lectores de tarjetas + display + buzzer + leds
\bigskip
...
\centering
  ...
  \end{center}
  \caption{Solución considerada 4}\label{Fig:HW} 
\end{figure}

\item[5 -] microcontrolador + lectores de tarjetas + display + buzzer + leds
\bigskip
...
\centering
  ...
  \end{center}
  \caption{Solución considerada 5}\label{Fig:HW} 
\end{figure}
...
El CL RC632 indica ciertos eventos estableciendo el bit IRQ en el registro PrimaryStatus, y además, por la activación del pin IRQ. La señal en el pin IRQ se puede utilizar para interrumpir un $\mu$-procesador. 
...
El CL RC632 informa al $\mu$-procesador sobre el origen de una interrupción mediante el establecimiento del bit adecuado en el registro InterruptRq. La relevancia de cada bit de petición de interrupción como fuente de una interrupción puede ser enmascarada con el bit de habilitación de interrupciones en el registro InterruptEn. 
